...
- `ENVIRONMENT`: `development` | `staging` | `production`. Staging **must** set `staging` (with `DEBUG=False` the default is `production`). Web and Celery workers must share the same value.
...
### Optional
- `SENTRY_DSN`: Error tracking (`SENTRY_ENVIRONMENT` defaults to `ENVIRONMENT`)
- `EMAIL_*`: Email configuration
- `AWS_*`: S3 storage configuration
- `USE_S3`: Enable S3 storage (`True`/`False`)
- `AWS_LOCATION`: Optional S3 key prefix override. Defaults: `incel-esign-dev` / `incel-esign-staging` / `incel-esign-app`. Non-production cannot use `incel-esign-app`.
